...
```rust
{{#include ../docs-router/src/doc_examples/untested_04/hooks_composed.rs:wrap_context}}
```
...
```rust
{{#include ../docs-router/src/doc_examples/untested_04/hooks_composed.rs:use_storage}}
```

## Custom Hook Logic

You can use [`cx.use_hook`](https://docs.rs/dioxus/~0.4/dioxus/prelude/struct.ScopeState.html#method.use_hook) to build your own hooks. In fact, this is what all the standard hooks are built on!
...
- The `use_state` hook tracks state in the hook value, and uses [`cx.schedule_update`](https://docs.rs/dioxus/~0.4/dioxus/prelude/struct.ScopeState.html#method.schedule_update) to make Dioxus re-render the component whenever it changes.

Here is a simplified implementation of the `use_state` hook:

```rust
{{#include ../docs-router/src/doc_examples/untested_04/hooks_custom_logic.rs:use_state}}
```

- The `use_context` hook calls [`cx.consume_context`](https://docs.rs/dioxus/~0.4/dioxus/prelude/struct.ScopeState.html#method.consume_context) (which would be expensive to call on every render) to get some context from the scope

Here is an implementation of the `use_context` and `use_context_provider` hooks:

```rust
{{#include ../docs-router/src/doc_examples/untested_04/hooks_custom_logic.rs:use_context}}
```
...
```rust
{{#include ../docs-router/src/doc_examples/untested_04/hooks_anti_patterns.rs:non_clone_state}}
```
