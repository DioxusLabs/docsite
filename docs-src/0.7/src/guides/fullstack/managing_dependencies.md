...
```rust
{{#include ../docs-examples/src/server_tokio_import.rs:tokio_import}}
```

You also need to only compile any usage of the dependency when the feature is enabled:
```rust
{{#include ../docs-examples/src/server_tokio_import.rs:tokio_usage}}
```
...
```rust
{{#include ../docs-examples/src/server_tokio_import.rs:tokio_module}}
```
