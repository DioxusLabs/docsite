...
\```rust, no_run
{{#include ../docs-router/src/doc_examples/meme_editor.rs:meme_component}}
\```
...
\```rust, no_run
{{#include ../docs-router/src/doc_examples/meme_editor.rs:caption_editor}}
\```
...
\```rust, no_run
{{#include ../docs-router/src/doc_examples/meme_editor.rs:meme_editor}}
\```
...
Dioxus offers a better solution than this "prop drilling" – providing context. The [`use_context_provider`](https://docs.rs/dioxus-hooks/~0.5/dioxus_hooks/fn.use_context_provider.html) hook provides any Clone context (including Signals!) to any child components. Child components can use the [`use_context`](https://docs.rs/dioxus-hooks/~0.5/dioxus_hooks/fn.use_context.html) hook to get that context and if it is a Signal, they can read and write to it.

First, we have to create a struct for our dark mode configuration:

\```rust, no_run
{{#include ../docs-router/src/doc_examples/meme_editor_dark_mode.rs:DarkMode_struct}}
\```

Now, in a top-level component (like `App`), we can provide the `DarkMode` context to all children components:

\```rust, no_run
{{#include ../docs-router/src/doc_examples/meme_editor_dark_mode.rs:context_provider}}
\```

As a result, any child component of `App` (direct or not), can access the `DarkMode` context.

\```rust, no_run
{{#include ../docs-router/src/doc_examples/meme_editor_dark_mode.rs:use_context}}
\```
...
\```rust, no_run
{{#include ../docs-router/src/doc_examples/meme_editor_dark_mode.rs:toggle}}
\```
