...
```rust, no_run
{{#include src/doc_examples/untested_04/input_controlled.rs:component}}
```
...
```rust, no_run
{{#include src/doc_examples/untested_04/input_uncontrolled.rs:component}}
```
```inject-dioxus
DemoFrame {
...
```rust, no_run
{{#include src/doc_examples/untested_04/input_fileengine.rs:component}}
```
...
```rust, no_run
{{#include src/doc_examples/untested_04/input_fileengine_async.rs:onchange_event}}
```

Lastly, this example shows you how to select a folder, by setting the `directory` attribute to `true`.

```rust, no_run
{{#include src/doc_examples/untested_04/input_fileengine_folder.rs:rsx}}
```
