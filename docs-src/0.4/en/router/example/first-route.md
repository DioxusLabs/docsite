...
```rust
{{#include src/doc_examples/untested_04/first_route.rs:home}}
```
...
```rust
{{#include src/doc_examples/untested_04/catch_all.rs:fallback}}
```

Next, register the route in the Route enum to match if all other routes fail.

```rust
{{#include src/doc_examples/untested_04/catch_all.rs:router}}
```
