...
You can use [`use_hook`](https://docs.rs/dioxus/~0.5/dioxus/prelude/fn.use_hook.html) to build your own hooks. In fact, this is what all the standard hooks are built on!
...
- The `use_signal` hook tracks state in the hook value, and uses [`schedule_update`](https://docs.rs/dioxus/~0.5/dioxus/prelude/fn.schedule_update.html) to make Dioxus re-render the component whenever it changes.
...
- The `use_context` hook calls [`consume_context`](https://docs.rs/dioxus/~0.5/dioxus/prelude/fn.consume_context.html) (which would be expensive to call on every render) to get some context from the component
