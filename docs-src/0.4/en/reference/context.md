...
```rust, no_run
{{#include src/doc_examples/untested_04/meme_editor.rs:meme_component}}
```
...
```rust, no_run
{{#include src/doc_examples/untested_04/meme_editor.rs:caption_editor}}
```
...
```rust, no_run
{{#include src/doc_examples/untested_04/meme_editor.rs:meme_editor}}
```
...
```rust, no_run
{{#include src/doc_examples/untested_04/meme_editor_dark_mode.rs:DarkMode_struct}}
```

Now, in a top-level component (like `App`), we can provide the `DarkMode` context to all children components:

```rust, no_run
{{#include src/doc_examples/untested_04/meme_editor_dark_mode.rs:context_provider}}
```

As a result, any child component of `App` (direct or not), can access the `DarkMode` context.

```rust, no_run
{{#include src/doc_examples/untested_04/meme_editor_dark_mode.rs:use_context}}
```
...
```rust, no_run
{{#include src/doc_examples/untested_04/meme_editor_dark_mode.rs:toggle}}
```
