...
```rust, no_run
{{#include src/doc_examples/untested_04/event_click.rs:rsx}}
```
```inject-dioxus
DemoFrame {
    event_click::App {}
...
```rust, no_run
{{#include src/doc_examples/untested_04/event_nested.rs:rsx}}
```
...
```rust, no_run
{{#include src/doc_examples/untested_04/event_prevent_default.rs:prevent_default}}
```
...
```rust, no_run
{{#include src/doc_examples/untested_04/event_handler_prop.rs:component_with_handler}}
```

Then, you can use it like any other handler:

```rust, no_run
{{#include src/doc_examples/untested_04/event_handler_prop.rs:usage}}
```
...
```rust, no_run
{{#include src/doc_examples/untested_04/event_handler_prop.rs:custom_data}}
```
