...
\```rust
{{#include ../docs-examples/src/server_function_desktop_client.rs:server_url}}
\```
...
\```rust
{{#include ../docs-examples/src/server_function_desktop_client.rs:server_function}}
\```

The `GetServerData` type has to be registered in the server, which will add the corresponding route to the server.
\```rust
{{#include ../docs-examples/src/server_function_desktop_client.rs:function_registration}}
\```
