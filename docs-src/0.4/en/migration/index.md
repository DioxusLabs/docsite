...
#[component]
fn BorrowedComp(name: ReadOnlySignal<String>) -> Element<'a> {
    rsx! {
        "Hello {name}"
    }
}
...
let copy_state = use_signal(|| 0);
let clone_shared_state = use_context_provider(|| Signal::new(String::from("Hello")));
let clone_local_state = use_signal(|| String::from("Hello"));
